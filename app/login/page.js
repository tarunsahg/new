    ...
    <div className="auth-split">
      <div className="auth-form">
        <div className="inner">
          ...
          <h1>Welcome back</h1>
          <p className="sub">Sign in to access your orders, license keys, and downloads.</p>
          {oauthErr && <div className="af-err" style={errStyle}>{oauthErr}</div>}
          <a href={`/api/auth/google?next=${encodeURIComponent(next)}`} className="oauth-btn">
          ...
          <div className="oauth-or"><span>or sign in with email</span></div>
          <form onSubmit={submit}>
            ...
            <button type="submit" disabled={busy} className="btn btn-primary btn-xl btn-block">{busy ? <><span className="spinner"/> Signing in…</> : 'Sign in →'}</button>
          </form>
          <div className="switch">New here? <Link href="/register">Create an account</Link></div>
        </div>
      </div>
      <div className="auth-promo">
        <span className="pill">Premium code marketplace</span>
        <h2>Build faster with production-ready code</h2>
        <p className="desc">Join 10,800+ developers shipping faster with our curated library of PHP, HTML, and JavaScript products.</p>
        <div className="perks">
          <div className="pk"><div className="ic">⚡</div><div><b>Instant delivery</b><span>License keys in your inbox the second you check out.</span></div></div>
          <div className="pk"><div className="ic">🔄</div><div><b>Lifetime updates</b><span>Free updates and bug fixes for every product, forever.</span></div></div>
          <div className="pk"><div className="ic">🛡️</div><div><b>14-day refund</b><span>If it doesn’t work for you, we’ll make it right.</span></div></div>
        </div>
      </div>
    </div>
  );

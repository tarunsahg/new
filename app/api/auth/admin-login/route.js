...
export async function POST(req) {
  return handle(async () => {
    ...
    const u = db().prepare('SELECT * FROM users WHERE email=? AND role=?').get(email.toLowerCase().trim(), 'admin');
    if (!u || !verifyPassword(password, u.password)) throw httpError(401, 'Invalid admin credentials');
